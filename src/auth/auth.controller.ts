...
const REPORTING_FRONTEND_URL = process.env.REPORTING_FRONTEND_URL || process.env.NEXT_PUBLIC_REPORTING_FRONTEND_URL || 'https://reporting-demo-system-frontend.pianat.ai';
...
function getAllowedEntryOrigins(): string[] {
  ...
  if (base.includes('127.0.0.1:3000')) {
    if (!list.includes('https://reporting-demo-system-frontend.pianat.ai')) list.push('https://reporting-demo-system-frontend.pianat.ai');
  }
  // In live UAT, allow both https and http for same host (e.g. https://reporting-demo-system-frontend.pianat.ai)
...
function isAllowedRedirectUri(uri: string): boolean {
  ...
  const altBase = base.includes('localhost:3000') ? 'http://127.0.0.1:3000' : base.includes('127.0.0.1:3000') ? 'https://reporting-demo-system-frontend.pianat.ai' : null;
